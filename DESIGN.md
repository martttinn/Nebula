...
components:
  ...
  nav-pill:
    ...
    padding: "{spacing.sm}"
  brand-logo:
    sourcePath: "/brand-logos/{name}.{svg|png}"
...
- `brand-logo`: wrapper centralizado para mostrar logos SVG o PNG de tecnologías, clientes o marcas externas desde `/public/brand-logos`. Debe usarse en modo `square` para símbolos compactos y sutiles, y en modo `rectangular` para wordmarks o logotipos completos, sin duplicar paths ni dimensiones hardcodeadas en cada sección.
...
- `process-timeline`: sección editorial full-width de proceso con cards alternadas, nodos ilustrados y path SVG serpenteante guiado por scroll
  ...
  En móvil, la guía vertical debe pasar por el centro horizontal real de los planetas, no por el centro del antiguo nodo circular. La guía debe terminar en el centro del último planeta y no prolongarse por debajo de su imagen. El copy de las cards mobile puede usar un clamp más generoso que en desktop para no mutilar demasiado pronto la lectura.
...
- `projects-showcase`: stage introductorio de portfolio con tres paneles encadenados
  ...
  En desktop, el scroll debe descansar mediante snap programático en cuatro estados cerrados de la timeline: primera card compacta ya asentada, primera card full-screen, segunda card full-screen tras empujar por completo a la primera y tercera card full-screen tras empujar por completo a la segunda. No debe permitir reposo en puntos intermedios donde dos paneles queden partidos en pantalla.
...
- `testimonials`: capa de prueba social con motion de stack inspirada en Golden Grama y estética Nebula
  ...
  En móvil y tablet, la sección replica ese mismo apilado sticky con footprint, escala y labio superior adaptados al viewport compacto; no debe volver a una lista vertical simple.
  ...
  Cada reseña visible se resuelve como una gran card dentro de ese stack sticky centrado, con footprint amplio, borde fino, grid interno `4/8` en desktop y una composición vertical equivalente en viewports compactos, siempre con una división estructural clara entre identidad y cita.
  La columna izquierda usa labels técnicos sobrios en `Lilac`, reglas finas y nombre del cliente en `Syne`; la derecha debe resolver la reseña visible como un único claim corto en `Syne`, sin subtítulo secundario bajo la cita. La reseña completa puede mantenerse como fuente repo-safe, pero no debe renderizarse entera si convierte la card en un bloque pesado.
