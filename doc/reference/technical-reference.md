...
- `components/layout/vercel-observability.tsx`: isla cliente que carga `@vercel/analytics` y `@vercel/speed-insights` por idle para no meter observabilidad no visual en el arranque inmediato
...
- `lib/`: helpers de metadata, URL absoluta, contacto público, idle browser, preferencias de motion reducida y utilidades compartidas
...
- `components/layout/lenis-provider.tsx`: integración global cliente de `Lenis` sobre el scroll raíz, con respeto por `prefers-reduced-motion` y carga imperativa por idle para dejar el primer arranque sin importar el paquete pesado de scroll suave ni reenvolver el arbol React
- `components/layout/scroll-progress-bar.tsx`: indicador global de progreso de scroll fijado al borde superior del viewport, con suavizado cliente ligero propio, render tipo scrollbar premium y escritura imperativa de `transform` para evitar renders React en cada frame
- `components/layout/navbar-staggered-menu.tsx`: isla cliente del navbar para móvil y tablet, basada en `StaggeredMenu`; su fallback inicial se pinta con Tailwind y el overlay real se carga por intencion/idle junto con su CSS
...
- `components/StaggeredMenu.css`: stylesheet del menú escalonado retemado al lenguaje dark-tech de Nebula, importado desde el componente dinamico para no formar parte del CSS critico inicial
- `components/animate-ui/primitives/buttons/button.tsx`: primitive base para interacciones de botón con `hoverScale` y `tapScale`, resuelta con CSS transforms para no importar Motion en CTAs above-the-fold
...
- `components/ui/preloader.tsx`: overlay de entrada con branding y barra de progreso para la home pública, coordinado con el readiness del hero y con salida acotada para no bloquear innecesariamente el primer contenido; las microanimaciones del loader se resuelven con CSS y estado minimo
- `components/ui/split-text.tsx`: primitive reusable inspirada en `SplitText` de React Bits para reveals escalonados por caracteres o palabras, con transitions CSS y callback temporalizado
- `components/ui/nebula-logo-animated.tsx`: logotipo animado reutilizado desde `nebula-legacy` para el preloader, con entrada y sheen basados en keyframes CSS
...
- `lib/browser-idle.ts`: helper compartido para programar trabajo no critico con `requestIdleCallback` cuando existe y fallback por timeout controlado
- `lib/use-prefers-reduced-motion.ts`: hook cliente ligero para leer `prefers-reduced-motion` sin importar `motion/react` en componentes que solo necesitan esa preferencia
...
- el root layout monta observabilidad de Vercel mediante `components/layout/vercel-observability.tsx`, que importa `Analytics` y `SpeedInsights` por idle tras el arranque visual; el runtime queda preparado para recoger page views, visitantes y métricas de rendimiento reales cuando exista despliegue en Vercel y Web Analytics esté habilitado en el dashboard
- las variants reutilizables de botón comparten ya una base importada desde `animate-ui`, con microescala en CSS y styling propio de Nebula en la capa wrapper
...
- el scroll raíz del sitio puede suavizarse con `Lenis`, con anclas internas habilitadas y offset para el navbar fijo; el paquete se carga por idle y se instancia de forma imperativa cuando no hay `prefers-reduced-motion`, por lo que la navegacion nativa queda como fallback inicial hasta que se active el suavizado. `LenisProvider` debe mantener estable el arbol React renderizado para no remontar la home ni repetir el `Preloader`. Cualquier superficie anidada que deba preservar interacción propia debe evaluarse con opt-out granular y nunca con bloqueo total si rompe la navegación natural del documento
- el root layout monta también un indicador global de progreso de scroll en el borde superior del viewport, tipo scrollbar fino, con línea base muy tenue, fill lilac/silver y una interpolación ligera propia para que el avance no quede duro ni dependiente del valor instantáneo del scroll; la animacion actual escribe `transform: scaleX(...)` sobre un ref para evitar renders React por frame
...
- en móvil y tablet, el navbar colapsa a lockup + hamburguesa, y usa un overlay escalonado fullscreen basado en `StaggeredMenu`; el toggle morfea a `X` dentro del componente diferido, y todos los destinos activos del navbar, incluido `Contactar`, se renderizan como links tipográficos del mismo sistema, con cierre por `Escape`, foco inicial dentro del panel y retorno de foco al trigger. Desktop mantiene la navegación inline y el CTA premium originales. El codigo pesado del overlay responsive, GSAP, Motion y su CSS se carga de forma diferida por intencion del usuario o idle tardio, manteniendo un trigger visual equivalente en el HTML inicial
- el navbar público deja solo el CTA `Contactar`, resuelto ahora sobre la primitive estándar `Button` como outlined transparente en reposo, con borde claro, hover blanco con texto negro, escala `1.05` y `BorderBeam` de Magic UI montado como hijo directo del link en desktop mediante Motion; con `prefers-reduced-motion`, el beam animado no se renderiza
...
- la base incluye observabilidad pública de Vercel mediante Web Analytics y Speed Insights cargados por idle; la recogida real depende de despliegue en Vercel, configuración activa del proyecto y tráfico real suficiente para estabilizar percentiles de campo
