...
- el navbar público deja solo el CTA `Contactar`, resuelto ahora sobre la primitive estándar `Button` como outlined transparente en reposo, con borde claro, hover blanco con texto negro, escala `1.05` y `BorderBeam` montado dentro del link en desktop mediante animacion CSS; con `prefers-reduced-motion`, el beam animado no se renderiza
